...
<aside class="fixed inset-y-0 left-0 z-40 hidden w-72 bg-white border-r border-slate-200 lg:flex lg:flex-col shadow-sm">
    <div class="flex h-16 items-center gap-3 border-b border-slate-100 px-6">
        ...
        <div>
            <p class="text-sm font-semibold uppercase tracking-wide text-splitwise-dark">Expense Splitter</p>
            <p class="text-xs text-slate-500">Shared balances</p>
        </div>
    </div>
    ...
    <nav class="flex-1 space-y-1 px-4 py-6">
        @foreach ($navItems as $item)
            <a href="{{ $item['href'] }}"
               class="group flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium transition duration-200 {{ $item['active'] ? 'bg-splitwise-light text-splitwise-dark font-semibold' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900' }}">
                <svg class="h-5 w-5 flex-none {{ $item['active'] ? 'text-splitwise' : 'text-slate-400 group-hover:text-slate-600' }}" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                    <path d="{{ $item['icon'] }}" />
                </svg>
                {{ $item['label'] }}
            </a>
...
<div x-show="sidebarOpen"
    ...
    <aside class="fixed inset-y-0 left-0 flex w-80 max-w-[86vw] flex-col bg-white shadow-xl">
...
<header class="sticky top-0 z-30 bg-white border-b border-slate-200 lg:pl-72 shadow-sm">
    <div class="flex h-16 items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
        <div class="flex items-center gap-3 w-full lg:w-auto">
            <button type="button"
                    class="rounded-xl border border-slate-200 bg-white p-2 text-slate-500 shadow-sm transition hover:bg-slate-50 hover:text-slate-700 lg:hidden"
            ...
            <div class="flex-1 lg:flex-none">
                <p class="text-sm font-semibold text-slate-900">{{ trim($__env->yieldContent('title')) ?: 'Dashboard' }}</p>
            </div>
        </div>

        <div class="flex items-center gap-3">
            <a href="{{ route('profile.edit') }}" class="hidden rounded-xl px-3 py-2 text-sm font-medium text-slate-600 transition hover:bg-slate-50 hover:text-slate-900 sm:inline-flex">
                Profile
            </a>
